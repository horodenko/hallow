...
- [~] Environment tileset — **in repo, terrain + peering bits assigned (2026-07-06); awaiting Daniel's visual verify.** `assets/tiles/forest_path_tileset.tres` now carries a full terrain set ("Match Corners and Sides" mode, terrain 0 = Dirt Path, 1 = Grass) with all 8 peering bits assigned on every tile — derived programmatically by pixel-sampling each tile's edges/corners (dirt=brown vs grass=green), 100% classification confidence, and verified headless: a scripted test strip paints 240 cells with zero seam mismatches (`scenes/TilesetTestStrip.tscn`). Known gaps found during analysis: (a) the atlas is missing the 2 "diagonal checkerboard" blob combos (opposite corners grass) — only matters if two paths ever cross corner-to-corner, harmless for the intro; (b) only ONE pure-grass tile (tile 22) despite the prompt asking for 2–3 variants — visible repetition risk on large grass fields; (c) full-dirt has 3 variants (tiles 21/33/34), good. Gotcha for anyone painting terrain from code: pass `ignore_empty_terrains = false` to `set_cells_terrain_connect`, else cells bordering empty map stay unresolved (every bit in this set is a real terrain; none are "empty"). Original resolution note kept below.
- [~] Environment tileset (original note) — **resolved what it IS; peering bits still needed.** The "Godot (3×3)" export is the real asset: 768×256 = 48 tiles at 64×64 (clean division). "3×3" refers to Godot's terrain peering-bit *mode*, not a tile count — a complete set legitimately needs this many tiles for all corner/edge/junction combinations. No metadata came with it, so every tile's Terrain + Peering Bits must be assigned by visual inspection in Godot's TileSet editor (Fable first-pass + Daniel spot-check via a small painted test strip). **Still needs adding to the repo** — only the old ambiguous 131×131 single tile is committed as of 2026-07-06. A numbered reference overlay exists (`tileset_48_labeled.png`) for referring to tiles by index; reference only, never a game asset.
...
- [~] Tree(s) — 2, now in the repo (`assets/objects/trees/`, added 2026-07-06). **Style flag:** the "large deciduous tree" is bare/leafless and dark — it clashes with the brief's sunlit "fruit, animals, life" garden and the reference's lush canopy. The intro map uses the leafy tree for all framing and places the bare one only sparsely in the far-east dusk zone (x > 3400) where the mood darkens — remove or regenerate a leafy variant if that reads wrong.
